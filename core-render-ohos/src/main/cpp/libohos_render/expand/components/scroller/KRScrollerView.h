...
struct KRNativeScrollWriteOperation {
    ...
    KRPoint target;
    bool animated = false;
    // Active-axis fact recorded at install: whether this write can physically start
    // native motion (and thus produce a deferred OnScrollStop). Gates replacement
    // stop arming; see KRScrollActiveAxisMotionExpected.
    bool native_motion_expected = false;
