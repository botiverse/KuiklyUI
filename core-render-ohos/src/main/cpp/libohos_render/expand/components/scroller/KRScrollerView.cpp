...
void KRScrollerView::SetContentOffset(const KRAnyValue &value, const KRRenderCallback &callback) {
    ...
    operation->native_motion_expected = KRScrollActiveAxisMotionExpected(
        direction_row_, current_offset.x, current_offset.y, offset_x, offset_y, animate);
    if ((std::fabs(current_offset.x - offset_x) <= 0.5f &&
         std::fabs(current_offset.y - offset_y) <= 0.5f) ||
        (animate && !operation->native_motion_expected)) {
        // Active-axis fact: for animated writes, OHOS only animates along the scroller
        // direction axis. If the target already matches the current offset on that
        // axis, no physical motion starts and no OnScrollStop will ever arrive, so the
        // write completes immediately instead of waiting for a phantom terminal.
    ...
    if (!animate && IsCurrentNativeScrollWrite(operation)) {
        ...
        KRContextScheduler::ScheduleTask(accepted_duration + slack, [weak_this, operation]() {
            if (auto strong_this = weak_this.lock()) {
                ...
                if (operation->native_motion_expected) {
                    ArkUI_NumberValue values[] = {{.f32 = 0}, {.f32 = 0}};
                    ArkUI_AttributeItem item = {values, 2};
...
std::shared_ptr<KRNativeScrollWriteOperation> KRScrollerView::InstallScrollWrite(
    ...
    if (previous && KRShouldStopReplacedScrollMotion(
        previous->native_motion_expected,
...
void KRScrollerView::SetContentInset(const std::shared_ptr<KRScrollerContentInset> &content_inset,
    ...
    if (content_inset->generation < 0 && native_phase_before_install != 0) {
        ...
        if (target_offset.x != current_offset.x || target_offset.y != current_offset.y) {
            ...
            operation->native_motion_expected = KRScrollActiveAxisMotionExpected(
                direction_row_, current_offset.x, current_offset.y, target_offset.x, target_offset.y, true);
            if (operation->native_motion_expected) {
                ...
                // Active-axis fact: the correction differs only off-axis, so OHOS will
                // not animate; treat the correction as finished instead of waiting for
                // a phantom stop.
...
void KRScrollerView::OnScrollFrameBegin(ArkUI_NodeEvent *event) {
    ...
    if (IsCurrentNativeScrollWrite(operation) &&
        ...
        operation->animated &&
        KRScrollActiveAxisDelta(direction_row_, point.x, point.y,
                                operation->start.x, operation->start.y) > 0.5f) {
...
void KRScrollerView::OnScrollStop(ArkUI_NodeEvent *event) {
    ...
    if (IsCurrentNativeScrollWrite(operation) && operation->animated && operation->observed_start) {
        ...
        // Active-axis fact: ArkUI can only reach the target on the scroller
        // direction axis; an admitted write (active diff > 0.5f) terminates when
        // the active axis reaches target, regardless of off-axis residue.
        if (KRScrollActiveAxisDelta(direction_row_, point.x, point.y,
                                    operation->target.x, operation->target.y) <= 1.0f) {
